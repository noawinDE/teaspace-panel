...
if(isset($_POST['answerTicket'])){
    if (isset($_POST['message']) && !empty($_POST['message'])) {
        ...
        if($user->getDataById($ticketInfos['user_id'],'support_notify') == 1) {
            include 'app/notifications/mail_templates/support/new_answer.php';
            sendMail($user->getDataById($ticketInfos['user_id'], 'email'), $user->getDataById($ticketInfos['user_id'], 'username'), $mailContent, $mailSubject);
        }
		header('refresh:3;url='.$url.'team/ticket/'.$ticket_id);
